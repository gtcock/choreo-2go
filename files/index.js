...
const NEZHA_KEY = process.env.NEZHA_KEY || '83973NDRVMfBU014Br';   // 没用哪吒，把这个参数空着
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'choreo.godtop.us.kg';     // 建议使用token，argo端口8080，cf后台设置需对应,使用json需上传json和yml文件至files目录
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiMDNmZDcwNjc2ZjgyMDA4MzVmYTViM2EyZjYxMDE2YzIiLCJ0IjoiNmY1N2JiYmEtM2MwZC00ZDBhLTkzMTQtOWNmMzBmODRhNDEzIiwicyI6IlpXWXhaVFUzWlRZdFltRXhPUzAwTXpBNUxUZzVOVFl0TkRBM01XRTJNVEUzTkRWayJ9';
...
function runServer() {
  ...
  if (ARGO_AUTH.match(/^[A-Z0-9a-z=]{120,250}$/)) {
    command2 = `./server tunnel --edge-ip-version auto --no-autoupdate --protocol http2 run --token ${ARGO_AUTH}`;
  } else {
    command2 = `./server tunnel --edge-ip-version auto --config tunnel.yml run`;
  }
