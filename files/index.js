...
const logFilePath = '/tmp/cloudflared.log'; // 日志文件路径
...
function runServer() {
  ...
  if (ARGO_AUTH.match(/^[A-Z0-9a-z=]{120,250}$/)) {
    command2 = `nohup ./server tunnel --edge-ip-version auto --no-autoupdate --protocol http2 run --token ${ARGO_AUTH} >${logFilePath} 2>&1 &`;
  } else {
    command2 = `nohup ./server tunnel --edge-ip-version auto --config tunnel.yml run >${logFilePath} 2>&1 &`;
  }

  exec(command2, (error) => {
    if (error) {
      console.error(`server running error: ${error}`);
      fs.appendFileSync(logFilePath, `\n[Error] server running error: ${error.message}\n`);
    } else {
